	...
	public function update_wishlist($wishlist_id,$p_id)
	{
		...
		if(!empty($product))
		{
			...
			if(empty($getolddata))
			{
				...
				$this->session->set_flashdata('message','<div class="alert alert-success">Item Add to Cart successfully.</div>');
				redirect($_SERVER['HTTP_REFERER']);
			}
			else
			{
				$this->db->delete("add_to_wishlist",array("id"=>$wishlist_id,));
				$this->session->set_flashdata('message','<div class="alert alert-success">Item Already Added......</div>');
				redirect($_SERVER['HTTP_REFERER']);
			}
