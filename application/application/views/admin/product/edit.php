...
function get_selected_size()
{
   ...
   $(select_size).each(function(index,item){
      size_old_names.push($(item).data('name'));
   });
   size_temp_old = all_image_stock[size_key];
   $(all_image_size[size_key]).each(function(index,item){
      if(size_old_names.includes(size_temp_old[index].size))
      {
         stock_html_old = stock_html_old+`
         <div class="cutom-stock">
            <label class="form-label">`+size_temp_old[index].size+`</label>
            <input type="number" class="form-control" name="stock`+size_key+`[]" value="`+size_temp_old[index].value+`">
         </div>
         `;
      }
   });
   $(select_size).each(function(index,item){
      if(all_image_size[size_key]!=undefined)
      {
         if(!all_image_size[size_key].includes(item.value))
         {
   ...
   $("#cutom-stock"+size_key).html(stock_html_old+stock_html);
}
